public class Queen implements Pieces {
  ...
  public void drawPiece() {
    if(white) {
      image(wq, x*50+50, 600-(y*50+50), 50, 50);
    } else {
      image(bq, x*50+50, 600-(y*50+50), 50, 50);
    }
  }
  
  @Override
  public boolean checkMove(int checkX, int checkY) {
    if (!((checkX==x)&&(checkY==y)) && checkX>=0 && checkX<=10 && checkY>=0 && checkY<=10 && (Math.abs(checkX-x)==Math.abs(checkY-y)||(checkX==x)||(checkY==y))) {
      return true;
    } else {
      return false;
    }
