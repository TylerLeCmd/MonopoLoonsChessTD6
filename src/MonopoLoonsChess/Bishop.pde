public class Bishop implements Pieces {
  ...
  public void drawPiece() {
    if(white) {
      image(wb, x*50+50, 600-(y*50+50), 50, 50);
    } else {
      image(bb, x*50+50, 600-(y*50+50), 50, 50);
    }
  }
  
  @Override
  public boolean checkMove(int checkX, int checkY) {
    if (!((checkX==x)&&(checkY==y)) && checkX>=0 && checkX<=10 && checkY>=0 && checkY<=10 && Math.abs(checkX-x)==Math.abs(checkY-y)) {
      return true;
    } else {
      return false;
    }
